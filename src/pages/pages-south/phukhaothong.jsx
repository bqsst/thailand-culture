...
const AppPhuKhaoThong = () => {
    ...
    return (
        <div>
            {detail.length > 0 ? (
                detail.slice(1,2).map((datas, index) => (
                    <div key={index} className="px-4 sm:px-8 md:px-[100px] xl:px-[200px] pt-[52px] sm:pt-[58px]  xl:pt-[70px]">
                        ...
                        />
                        <AppListBut name={datas.name} map={datas.locationSrc} describe={"Explore charm of Phukhaothong"} />
                    </div>
                ))
            ) : (
...
export default AppPhuKhaoThong;
